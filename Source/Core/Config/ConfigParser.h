...
public:
    ...
    std::map<std::string, Value> GetAllValuesUnderPath(const std::string &_PathPrefix) const;

    // Convert path-based values to nested JSON
    nlohmann::json ConvertPathsToJson(const std::map<std::string, Value> &_Values) const;
