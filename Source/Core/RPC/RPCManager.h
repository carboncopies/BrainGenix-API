...
public:
     ...
     * 
     * Initializes the RPCManager with pointers to the configuration object, logging system, and server instance.
     * 
     ...
     * 
     * Registers a handler function for a specific route in the RPC server.
     * 
     * @param _RouteHandle The name of the route to register.
     * @param _Function The handler function to associate with the route.
     */
    void AddRoute(std::string _RouteHandle, std::function<std::string(std::string _JSONRequest)> _Function);

    /**
     * @brief Handles a generic JSON-based API request for the NES system.
     * 
     * Processes the request and optionally overrides the simulation ID.
     * 
     * @param _JSONRequest The JSON request string to process.
     * @param _SimulationIDOverride Optional simulation ID to override (default is -1).
     * @return std::string The response string generated by the request handler.
     */
    std::string NESRequest(std::string _JSONRequest, int _SimulationIDOverride = -1);

    /**
     * @brief Handles a generic JSON-based API request for the EVM system.
     * 
     * Processes the request and optionally overrides the simulation ID.
     * 
     * @param _JSONRequest The JSON request string to process.
     ...
     * @return std::string The response string generated by the request handler.
     */
    std::string EVMRequest(std::string _JSONRequest, int _SimulationIDOverride = -1);
