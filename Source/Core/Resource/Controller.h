...
class BrainGenixAPIController : public oatpp::web::server::api::ApiController {
  ...
  BG::API::VSDA::Manager* VSDAManager_;
public:
  BrainGenixAPIController(BG::API::Server::Server* _Server, BG::API::RPC::Manager* _Manager, BG::API::VSDA::Manager* _VSDAManager, OATPP_COMPONENT(std::shared_ptr<ObjectMapper>, objectMapper))
...
ENDPOINT("POST", "/Auth/GetToken", token, BODY_DTO(Object<LoginDTO>, loginDTO)) {
    try {
        // 1. Validate input
...
// Add VSDA-related HTTP endpoints
ENDPOINT("GET", "/api/vsda/status", GetVSDAStatus) {
  if (!VSDAManager_) {
      auto dto = MessageDto::createShared();
      dto->statusCode = 500;
      dto->message = "VSDA Manager not available";
      return createDtoResponse(Status::CODE_500, dto);
  }
  
  auto status = VSDAManager_->GetClusterStatus();
  auto dto = MessageDto::createShared();
  dto->statusCode = 200;
  dto->message = status.dump();
  return createDtoResponse(Status::CODE_200, dto);
}

ENDPOINT("GET", "/api/vsda/nodes", GetVSDANodes) {
  if (!VSDAManager_) {
      auto dto = MessageDto::createShared();
      dto->statusCode = 500;
      dto->message = "VSDA Manager not available";
  ...
  auto nodes = VSDAManager_->GetNodes();
  auto dto = MessageDto::createShared();
  dto->statusCode = 200;
  dto->message = nodes.dump();
  return createDtoResponse(Status::CODE_200, dto);
}

ENDPOINT("GET", "/api/vsda/leader", GetVSDALeader) {
  if (!VSDAManager_) {
      auto dto = MessageDto::createShared();
      dto->statusCode = 500;
      dto->message = "VSDA Manager not available";
  ...
  auto leader = VSDAManager_->GetLeader();
  auto dto = MessageDto::createShared();
  dto->statusCode = 200;
  dto->message = leader.ToJSON().dump();
  return createDtoResponse(Status::CODE_200, dto);
}

ENDPOINT("GET", "/api/vsda/stats", GetVSDASats) {
  if (!VSDAManager_) {
      auto dto = MessageDto::createShared();
      dto->statusCode = 500;
      dto->message = "VSDA Manager not available";
  ...
  nlohmann::json stats;
  stats["node_count"] = VSDAManager_->CountNodes();
  stats["has_leader"] = VSDAManager_->HasLeader();
  stats["leader_id"] = VSDAManager_->HasLeader() ? VSDAManager_->GetLeader().id : "none";
  
  auto dto = MessageDto::createShared();
  dto->statusCode = 200;
  dto->message = stats.dump();
  return createDtoResponse(Status::CODE_200, dto);
}
