...
void Route::RouteCallback(const std::shared_ptr<restbed::Session> _Session) {
    ...
    bool UpstreamStatus = Util::NESQueryJSON(Server_->NESClient, Server_->IsNESClientHealthy_, "Tool/PatchClampDAC/Create", UpstreamQuery.dump(), &UpstreamResponseStr);
    if (!UpstreamStatus) {
    ...
    //Logger_->Log("Creating PatchClampDAC with ID "+ std::to_string(static_cast<int>(Response["PatchClampDACID"]))+'\n',1);

    //Util::SendJSON(_Session.get(), &Response);

    Logger_->Log("Creating PatchClampDAC: "+ UpstreamResponseStr+'\n',1);
