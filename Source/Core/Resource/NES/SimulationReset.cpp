...
Route::Route(Server::Server *_Server, restbed::Service &_Service) {
  ...
  RouteResource->set_path("/NES/Simulation/Reset");
...
void Route::RouteCallback(const std::shared_ptr<restbed::Session> _Session) {
    ...
    bool UpstreamStatus = Util::NESQueryJSON(Server_->NESClient, Server_->IsNESClientHealthy_, "Simulation/Reset", UpstreamQuery.dump(), &UpstreamResponseStr);
    if (!UpstreamStatus) {
    ...
    std::cout<<"Resetting Simulation With ID "<<Request->get_query_parameter("SimulationID", -1)<<std::endl;
