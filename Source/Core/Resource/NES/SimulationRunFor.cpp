...
Route::Route(Server::Server *_Server, restbed::Service &_Service) {
  ...
  RequiredParams_.push_back("Runtime_ms");
  ...
  RouteResource->set_path("/NES/Simulation/RunFor");
...
void Route::RouteCallback(const std::shared_ptr<restbed::Session> _Session) {
    ...
    UpstreamQuery["Runtime_ms"] = Request->get_query_parameter("Runtime_ms", 0.0f);
    

    std::string UpstreamResponseStr = "";
    bool UpstreamStatus = Util::NESQueryJSON(Server_->NESClient, Server_->IsNESClientHealthy_, "Simulation/RunFor", UpstreamQuery.dump(), &UpstreamResponseStr);
    if (!UpstreamStatus) {
    ...
    std::cout<<"Running Simulation With ID "<<Request->get_query_parameter("SimulationID", -1)<<std::endl;
