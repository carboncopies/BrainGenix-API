...
int main(int NumArguments, char** ArguemntValues) {
    ...
    YAML::Node Config;
    bool Status = BrainGenix::BG::Module::LoadLocalConfiguration("Config.yaml", Config);
    ...
    BrainGenix::BG::Module::ArgumentParser ArgumentParser = BrainGenix::BG::Module::ArgumentParser(SystemUtils->Logger_.get());
    ...
    SystemUtils->Logger_->Log("Instantiating BG Project Loader Pointer", 4);
    ProjectUtils->ProjectLoader_ = std::make_unique<ERS_CLASS_ProjectLoader>(SystemUtils.get());

    SystemUtils->Logger_->Log("Instantiating BG Project Writer Pointer", 4);
    ProjectUtils->ProjectWriter_ = std::make_unique<ERS_CLASS_ProjectWriter>(SystemUtils.get());

    SystemUtils->Logger_->Log("Instantiating BG Project Manager Pointer", 4);
    ...
    SystemUtils->Logger_->Log("Instantiating BG Model Importer", 4);
    ...
    SystemUtils->Logger_->Log("Instantiating BG Controller Input Manager", 5);
